...
int send_message(int type, PacketStorage message) {
	...
	if(type & DASHBOARD){
		...
		// assign IP, PORT
		servaddr.sin_family = AF_INET;
		servaddr.sin_addr.s_addr = inet_addr(dashboard_device.ip_address);
		servaddr.sin_port = htons(PORT);
		
		if (connect(client_socket, (SA *) &servaddr, sizeof(servaddr)) != 0) {
			printf("CLIENT: connection with the server failed...\n");
		} else {
			printf("CLIENT: connected to dashboard...\n");
			//pack_to_dashboard = malloc(128);
			bzero(pack_to_dashboard, 256);
			strncat(pack_to_dashboard, dashboard_device.ip_address, 15);
			strncat(pack_to_dashboard, ";", 2);
			strncat(pack_to_dashboard, "192.168.47.38", 15);
			strncat(pack_to_dashboard, ";", 2);
			PacketStorage_read(message, pack_to_dashboard);
			
			//printf("Send packet 2 dashboard %ld\n", sizeof(pack_to_dashboard));
			//printf("Message: %s\n", pack_to_dashboard);
			if(write(client_socket, &pack_to_dashboard, 256) < 0)
				printf("CLIENT: Error message to Dashboard\n");
			//free(pack_to_dashboard);
		}
	}
