...
void init_communications(void) {
    ...
    strncpy(list_devices[0].ip_address,"192.168.47.30", 14);
    list_devices[0].type = FACTORY;
    list_devices[0].isConnected = 1;
    strncpy(list_devices[1].ip_address , "192.168.47.32", 14);
    list_devices[1].type = FACTORY;
    list_devices[1].isConnected = 1;
    strncpy(list_devices[2].ip_address , "192.168.47.34", 14);
    list_devices[2].type = FACTORY;
    list_devices[2].isConnected = 1;
    strncpy(list_devices[3].ip_address , "192.168.47.36", 14);
    list_devices[3].type = FACTORY;
    list_devices[3].isConnected = 1;
    strncpy(list_devices[4].ip_address ,"192.168.47.38", 14);
    list_devices[4].type = FACTORY;
    list_devices[4].isConnected = 1;
    strncpy(list_devices[5].ip_address,"192.168.47.40", 14);
    list_devices[5].type = FACTORY;
    list_devices[5].isConnected = 1;
    
    strncpy(dashboard_device.ip_address,"192.168.47.28", 14);
    dashboard_device.type = DASHBOARD;
    dashboard_device.isConnected = 1;
	
    //_getDeviceIP("ifconfig | grep \"inet 192.\" | awk '{print $2}'", ip);
    ////strcpy(device.ip_address, ip);
    printf("IP: %s\n", dashboard_device.ip_address);
    
    strncpy(device.ip_address,"192.168.47.38", 14);
    device.type = FACTORY;
	...
	/*for(int i = 25; i < 50; i++){
		...
		strcat(msg, "P_IP:");
		strcat(msg, device.ip_address);
		if(strcmp(ip, device.ip_address) != 0){
			_send_message(ip, FACTORY, msg);
			//send_message(ip, DASHBOARD, msg);
		}
	}
