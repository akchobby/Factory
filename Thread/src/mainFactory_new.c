...
PacketStorage pck_s, output_pck, periodic_pck;
...
void* ExecOperation(void *no_arg) {
	...
	while(1) {
		...
		if(strcmp(commandData->cmd, "S_CMS_D") == 0) {
			...
			response_aperiodic_actuator(led_st, buffer_out);
			...
			response_aperiodic_actuator(relay_st, buffer_out);
			...
			response_aperiodic_actuator(lcd_st, buffer_out);
			...
			//sem_post(&sem_EO_AL);
			getAlarm(ala_st);
			response_aperiodic_actuator(ala_st, buffer_out);
...
void* SendDataPeriodic(void* period_str) {
	...
	while(1) {
		char *buffer_out = malloc(BUFFER_SIZ*sizeof(char));
		// UPDATE TIME
...
int main(void) {
	...
	output_pck = PacketStorage_init();
	periodic_pck = PacketStorage_init();
